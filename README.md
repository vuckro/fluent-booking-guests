...
3. No configuration needed — works automatically on all pages with a FluentBooking widget.
...
2. Updates each `<tbody>` item row: sets the quantity cell and recalculates `unitPrice × totalGuests`.
3. Updates the `<tfoot>` total via the `.fcal_payment_amount` span.
4. Sets `required` on all name/email inputs inside guest rows.

Price parsing handles multiple locale formats (`15,00 CHF`, `$15.00`, `1.234,56`).

### Backend (`cooms-fluent-multireservation.php`)

The `fluent_booking/booking_data` filter (priority 20) sets `$bookingData['quantity']` to `count($bookingData['email'])` for multi-guest events when the core hasn't set it yet. This guards against edge cases in custom payment flows.
...
### 2.0.0
- Rewritten: simplified codebase (~100 lines JS, ~45 lines PHP).
...
- Removed: email auto-fill injection (unnecessary complexity).
